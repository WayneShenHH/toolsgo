// Package logger log interface with logrus
package logger
...
import (
	...
	"github.com/WayneShenHH/toolsgo/pkg/module/logger/stackdriver"
	"github.com/WayneShenHH/toolsgo/pkg/module/version"
)
...
func Init() {
	...
	// 設定輸出 format
	if strings.ToLower(environment.Setting.Logger.Formatter) == "stackdriver" {
		std.Formatter = stackdriver.NewFormatter(
			stackdriver.WithService(version.Name()),
			stackdriver.WithVersion(version.Format()),
		)
	}

	switch strings.ToLower(environment.Setting.Logger.StdLevel) {
	case "debug":
		std.Level = logrus.DebugLevel
	case "info":
